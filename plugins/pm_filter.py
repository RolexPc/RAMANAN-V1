...
async def qualities_check(bot, query):
    _, userid, qualities = query.data.split("#")
    if int(userid) not in [query.from_user.id, 0]:
        return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    if qualities == "unknown":
        return await query.answer("Sᴇʟᴇᴄᴛ ᴀɴʏ qualities ғʀᴏᴍ ᴛʜᴇ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴs !", show_alert=True)
    movie = temp.KEYWORD.get(query.from_user.id)
    if not movie:
        return await query.answer(script.OLD_ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    if qualities != "home":
        movie = f"{movie} {qualities}"
    files, offset, total_results = await get_search_results(query.message.chat.id, movie, offset=0, filter=True)
    if files:
        settings = await get_settings(query.message.chat.id)
        temp.SEND_ALL_TEMP[query.from_user.id] = files
...
async def select_language(bot, query):
    ...
    if int(userid) not in [query.from_user.id, 0]:
        return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    btn = [[
        InlineKeyboardButton("Sᴇʟᴇᴄᴛ Yᴏᴜʀ Dᴇꜱɪʀᴇᴅ Lᴀɴɢᴜᴀɢᴇ ↓", callback_data=f"qual#{userid}#unknown")
    ],[
        InlineKeyboardButton("360P", callback_data=f"qual#{userid}#360"),
        InlineKeyboardButton("480P", callback_data=f"qual#{userid}#480"),
        InlineKeyboardButton("520P", callback_data=f"qual#{userid}#520")
    ],[
        InlineKeyboardButton("720P", callback_data=f"qual#{userid}#720"),
        InlineKeyboardButton("1080P", callback_data=f"qual#{userid}#1080"),
        InlineKeyboardButton("1440P", callback_data=f"qual#{userid}#1440")
    ],[
        InlineKeyboardButton("2160P", callback_data=f"qual#{userid}#2160")  
    ],[
        InlineKeyboardButton("Gᴏ Bᴀᴄᴋ", callback_data=f"qual#{userid}#home")
    ]]
    try:
        await query.edit_message_reply_markup(
            reply_markup=InlineKeyboardMarkup(btn)
